use crate::{CVBytes, CVWords, BLOCK_LEN, IV, MSG_SCHEDULE, OUT_LEN};
...
fn compress_pre(
    ...
    let block_words = crate::words_from_le_bytes_64(block);
...
pub fn compress_xof(
    ...
    state[14] ^= cv[6];
    state[15] ^= cv[7];
    crate::le_bytes_from_words_64(&state)
}
...
pub fn hash1<const N: usize>(
    ...
    *out = crate::le_bytes_from_words_32(&cv);
}
